...
// BufferService manages open file buffers - opening, closing, reading/writing content, and assets.
class BufferService : public QObject, private Noncopyable {
  Q_OBJECT

public:
  // Constructor receives VxCore context handle via dependency injection.
  explicit BufferService(VxCoreContextHandle p_context, QObject *p_parent = nullptr);
  ~BufferService();
