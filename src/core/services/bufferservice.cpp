...
BufferService::BufferService(VxCoreContextHandle p_context, QObject *p_parent)
...
// Private methods.
bool BufferService::checkContext() const {
  if (!m_context) {
    qWarning() << "BufferService: VxCore context not initialized";
    return false;
  }
  return true;
}

QString BufferService::cstrToQString(char *p_str) {
  if (!p_str) {
    return QString();
  }
...
QJsonObject BufferService::parseJsonObjectFromCStr(char *p_str) {
  if (!p_str) {
    return QJsonObject();
  }
...
QJsonArray BufferService::parseJsonArrayFromCStr(char *p_str) {
  if (!p_str) {
    return QJsonArray();
  }
  QJsonParseError error;
