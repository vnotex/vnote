...
bool HookManager::doAction(const QString &p_hook, const NodeMoveEvent &p_event) {
  return doAction(p_hook, p_event.toVariantMap());
}

bool HookManager::doAction(const QString &p_hook, const NodeTransferEvent &p_event) {
  return doAction(p_hook, p_event.toVariantMap());
}
...
bool HookManager::doAction(const QString &p_hook, const FileExternalChangeEvent &p_event) {
  return doAction(p_hook, p_event.toVariantMap());
}

bool HookManager::doAction(const QString &p_hook, const ImageHostUploadEvent &p_event) {
  return doAction(p_hook, p_event.toVariantMap());
}

bool HookManager::doAction(const QString &p_hook, const ImageHostRemoveEvent &p_event) {
  return doAction(p_hook, p_event.toVariantMap());
}
